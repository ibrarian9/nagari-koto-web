...
<div x-show="tab === 'dip'" x-transition>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div class="bg-white rounded-2xl border border-gray-150 p-4 shadow-xs cursor-pointer hover:border-blue-200 transition-colors" @click="sub = 'info_berkala'">
        ...
        <div class="bg-white rounded-2xl border border-gray-150 p-4 shadow-xs cursor-pointer hover:border-emerald-200 transition-colors" @click="sub = 'info_setiapsaat'">
            <div class="flex justify-between items-start">
        ...
        <div class="bg-white rounded-2xl border border-gray-150 p-4 shadow-xs cursor-pointer hover:border-amber-200 transition-colors" @click="sub = 'info_sertamerta'">
            <div class="flex justify-between items-start">
    ...
    <!-- Sub-tab Switcher -->
    <div class="flex gap-1.5 bg-gray-100 p-1 rounded-xl border border-gray-200/50 w-fit max-w-full overflow-x-auto scrollbar-none mb-6">
        @foreach ([
            ['key' => 'info_berkala', 'label' => 'Info Berkala', 'icon' => 'schedule'],
            ['key' => 'info_setiapsaat', 'label' => 'Info Setiap Saat', 'icon' => 'folder_open'],
            ['key' => 'info_sertamerta', 'label' => 'Info Serta Merta', 'icon' => 'campaign'],
        ] as $s)
    ...
    <div class="card p-4 mb-6">
        <div class="relative">
            ...
            <input type="text" wire:model.live.debounce.300ms="infoSearch" class="form-input w-full pl-10" placeholder="Cari dokumen atau pengumuman...">
        </div>
    </div>
    ...
    <div class="space-y-6">
            ...
            <div class="space-y-3">
                @forelse ($berkalaItems as $item)
                    <div class="card p-5 flex flex-col sm:flex-row sm:items-center gap-4 hover:-translate-y-0.5 transition-all duration-200">
                        ...
                        <button wire:click="downloadBerkala({{ $item->id }})" class="btn-primary btn-sm whitespace-nowrap flex items-center gap-1.5">
            ...
            <div class="space-y-3">
                @forelse ($setiapSaatItems as $item)
                    <div class="card p-5 flex flex-col sm:flex-row sm:items-center gap-4 hover:-translate-y-0.5 transition-all duration-200">
                        ...
                        <button wire:click="downloadSetiapSaat({{ $item->id }})" class="btn-primary btn-sm whitespace-nowrap flex items-center gap-1.5">
                            <span class="material-symbols-outlined text-sm">download</span>
                            Download
                        </button>
                    </div>
        ...
        @if ($dikecualikan->content || $dikecualikan->attachment)
            <div class="card p-6 md:p-8">
                <div class="flex items-center gap-3 mb-4">
                    <div class="h-12 w-12 rounded-xl bg-gray-600 flex items-center justify-center">
                        <span class="material-symbols-outlined text-white text-2xl">lock</span>
                    </div>
                    <h2 class="text-xl font-bold text-gray-900">Informasi yang Dikecualikan</h2>
                </div>
                @if ($dikecualikan->content)
                    <div class="prose max-w-none text-gray-700 leading-relaxed whitespace-pre-line">{{ $dikecualikan->content }}</div>
                @endif
                @if ($dikecualikan->attachment)
                    <div class="mt-4 bg-gray-100 rounded-xl" style="height: 60vh; min-height: 400px;">
                        <iframe src="{{ Storage::url($dikecualikan->attachment) }}#toolbar=1&navpanes=0" class="w-full h-full border-0 rounded-xl" loading="lazy"></iframe>
                    </div>
                @endif
            </div>
        @endif
...
<div x-show="tab === 'pelayanan'" x-transition>
    <div class="flex gap-1.5 bg-gray-100 p-1 rounded-xl border border-gray-200/50 w-fit max-w-full overflow-x-auto scrollbar-none mb-6">
        @foreach ([
            ['key' => 'alur_info', 'label' => 'Alur Informasi', 'icon' => 'route'],
            ['key' => 'permohonan', 'label' => 'Permohonan Online', 'icon' => 'edit_note'],
            ['key' => 'alur_kbr', 'label' => 'Alur Keberatan', 'icon' => 'report'],
            ['key' => 'keberatan', 'label' => 'Keberatan Online', 'icon' => 'feedback'],
            ['key' => 'alur_skt', 'label' => 'Alur Sengketa', 'icon' => 'balance'],
        ] as $s)
            <button @click="sub = '{{ $s['key'] }}'"
                :class="sub === '{{ $s['key'] }}' ? 'bg-white text-desa-700 shadow-xs' : 'text-gray-600 hover:text-gray-900'"
                class="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-semibold whitespace-nowrap transition-all duration-150 active:scale-[0.98]">
    ...
    <div x-show="sub === 'alur_info'">
        <div class="card p-6 md:p-8">
            ...
            @if ($alurInformasi->content)
                <div class="prose max-w-none text-gray-700 leading-relaxed whitespace-pre-line">{{ $alurInformasi->content }}</div>
            @endif
            @if ($alurInformasi->image)
                <div class="mt-4" x-data="{ showLightbox: false }">
                    <div class="rounded-xl overflow-hidden border border-gray-100 cursor-pointer group relative" @click="showLightbox = true">
                        <img src="{{ Storage::url($alurInformasi->image) }}" alt="Alur Informasi" class="w-full object-contain max-h-80" loading="lazy">
                    ...
                    <p class="text-xs text-gray-400 mt-2 flex items-center gap-1"><span class="material-symbols-outlined text-xs">touch_app</span> Klik gambar untuk memperbesar</p>
                    {{-- Lightbox --}}
                    <div x-show="showLightbox" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" @click="showLightbox = false" @keydown.escape.window="showLightbox = false" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 cursor-zoom-out" style="display: none;">
                        <img src="{{ Storage::url($alurInformasi->image) }}" alt="Alur Informasi" class="max-w-full max-h-[90vh] object-contain rounded-xl shadow-2xl" @click.stop>
    ...
    <div x-show="sub === 'permohonan'">
        @if ($pmhSubmitted)
            <div class="card p-8 text-center">
                ...
                <div class="inline-flex items-center gap-2 px-6 py-3 bg-desa-50 rounded-xl border-2 border-desa-200 mb-6">
                ...
                <div class="flex flex-wrap justify-center gap-3">
                    <a href="{{ route('ppid.cek-status') }}" wire:navigate class="btn-primary"><span class="material-symbols-outlined text-base">search</span> Cek Status</a>
                ...
                <form wire:submit="submitPermohonan" class="space-y-5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" wire:model="pmhNama" class="form-input w-full" placeholder="Nama lengkap">
                            @error('pmhNama')<p class="form-error">{{ $message }}</p>@enderror
                        </div>
                        <div>
                            <label class="form-label">NIK (16 digit) <span class="text-red-500">*</span></label>
                            <input type="text" wire:model="pmhNik" class="form-input w-full" maxlength="16" placeholder="Nomor Induk Kependudukan">
                            @error('pmhNik')<p class="form-error">{{ $message }}</p>@enderror
                        </div>
                        <div>
                            <label class="form-label">Nomor Telepon <span class="text-red-500">*</span></label>
                            <input type="text" wire:model="pmhTelepon" class="form-input w-full" placeholder="08xxxxxxxxxx">
                            @error('pmhTelepon')<p class="form-error">{{ $message }}</p>@enderror
                        </div>
                        <div>
                            <label class="form-label">Email <span class="text-gray-400 text-xs">(opsional)</span></label>
                            <input type="email" wire:model="pmhEmail" class="form-input w-full" placeholder="email@contoh.com">
                        </div>
                    </div>
                    <div>
                        ...
                        @error('pmhAlamat')<p class="form-error">{{ $message }}</p>@enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea wire:model="pmhInfoDiminta" class="form-input w-full" rows="3" placeholder="Jelaskan informasi yang Anda butuhkan..."></textarea>
                        @error('pmhInfoDiminta')<p class="form-error">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="form-label">Tujuan Penggunaan Informasi <span class="text-red-500">*</span></label>
                        <textarea wire:model="pmhTujuan" class="form-input w-full" rows="2" placeholder="Untuk apa informasi ini digunakan..."></textarea>
                        @error('pmhTujuan')<p class="form-error">{{ $message }}</p>@enderror
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select wire:model="pmhFormat" class="form-input w-full">
                                @foreach($formatOptions as $k => $v)<option value="{{ $k }}">{{ $v }}</option>@endforeach
                            </select>
                        </div>
                        <div>
                            ...
                            <select wire:model="pmhCara" class="form-input w-full">
                                @foreach($caraOptions as $k => $v)<option value="{{ $k }}">{{ $v }}</option>@endforeach
                            </select>
                        </div>
                    </div>
                    <div x-data="{ previewUrl: null }" class="space-y-2">
                        ...
                        <label class="block cursor-pointer">
                            <div class="flex items-center gap-3 px-4 py-3 rounded-xl border-2 border-dashed border-gray-200 hover:border-desa-400 bg-gray-50 hover:bg-desa-50/30 transition-all text-sm text-gray-500">
                                ...
                                <div><span class="font-medium text-gray-700" x-text="previewUrl ? 'Ganti foto' : 'Upload foto KTP'">Upload foto KTP</span><p class="text-xs text-gray-400 mt-0.5">JPG, PNG, WebP. Maks 2MB</p></div>
                            </div>
                            <input type="file" wire:model="pmhLampiran" accept="image/*" class="sr-only" x-on:change="const f=$event.target.files[0]; if(f){const r=new FileReader(); r.onload=e=>previewUrl=e.target.result; r.readAsDataURL(f);}">
                        </label>
                        <template x-if="previewUrl"><img :src="previewUrl" class="w-full max-h-40 object-contain rounded-lg border border-gray-200 bg-white p-1" alt="Preview"></template>
                        <div wire:loading wire:target="pmhLampiran" class="text-xs text-desa-600 flex items-center gap-1"><span class="material-symbols-outlined text-sm animate-spin">progress_activity</span> Mengunggah...</div>
                        @error('pmhLampiran')<p class="form-error">{{ $message }}</p>@enderror
                    </div>
                    <button type="submit" class="btn-primary w-full justify-center" wire:loading.attr="disabled">
                        <span wire:loading.remove wire:target="submitPermohonan" class="flex items-center gap-2"><span class="material-symbols-outlined text-base">send</span> Kirim Permohonan</span>
                        <span wire:loading wire:target="submitPermohonan" class="flex items-center gap-2"><span class="material-symbols-outlined text-base animate-spin">progress_activity</span> Mengirim...</span>
                    </button>
                </form>
            </div>
        @endif
    ...
    <div x-show="sub === 'alur_kbr'">
        <div class="card p-6 md:p-8">
            ...
            @if ($alurKeberatan->content)
                <div class="prose max-w-none text-gray-700 leading-relaxed whitespace-pre-line">{{ $alurKeberatan->content }}</div>
            @endif
            @if ($alurKeberatan->image)
                <div class="mt-4" x-data="{ showLightbox: false }">
                    <div class="rounded-xl overflow-hidden border border-gray-100 cursor-pointer group relative" @click="showLightbox = true">
                        <img src="{{ Storage::url($alurKeberatan->image) }}" alt="Alur Keberatan" class="w-full object-contain max-h-80" loading="lazy">
                        <div class="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors duration-200 flex items-center justify-center">
                            <span class="material-symbols-outlined text-white text-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-200 drop-shadow-lg">zoom_in</span>
                        </div>
                    </div>
                    ...
                    {{-- Lightbox --}}
                    <div x-show="showLightbox" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" @click="showLightbox = false" @keydown.escape.window="showLightbox = false" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 cursor-zoom-out" style="display: none;">
                        <img src="{{ Storage::url($alurKeberatan->image) }}" alt="Alur Keberatan" class="max-w-full max-h-[90vh] object-contain rounded-xl shadow-2xl" @click.stop>
                        <button @click="showLightbox = false" class="absolute top-4 right-4 h-10 w-10 rounded-full bg-white/20 hover:bg-white/30 flex items-center justify-center text-white transition-colors">
                            <span class="material-symbols-outlined">close</span>
                        </button>
                    </div>
            ...
            <div class="card p-6 md:p-8">
                ...
                <form wire:submit="submitKeberatan" class="space-y-5">
                    <div>
                        ...
                        <input type="text" wire:model="kbrNoReg" class="form-input w-full" placeholder="cth: PMH-20260601-0001">
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            @error('kbrNama')<p class="form-error">{{ $message }}</p>@enderror
                        </div>
                        <div>
                            ...
                            @error('kbrNoHp')<p class="form-error">{{ $message }}</p>@enderror
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        @error('kbrAlamat')<p class="form-error">{{ $message }}</p>@enderror
                    </div>
                    <div>
                    ...
                    <div>
                        <label class="form-label">Alasan Pengajuan Keberatan <span class="text-red-400">*</span></label>
                        <div class="space-y-2 mt-2">
                            @foreach (\App\Models\PpidKeberatan::ALASAN as $k => $l)
                                <label class="flex items-start gap-3 p-3 rounded-lg border border-gray-200 hover:bg-gray-50 cursor-pointer transition-colors">
                                    <input type="radio" wire:model="kbrAlasan" value="{{ $k }}" class="mt-0.5 text-desa-600 focus:ring-desa-500">
                                    <span class="text-sm text-gray-700">{{ $l }}</span>
                                </label>
                            @endforeach
                        </div>
                        @error('kbrAlasan')<p class="form-error">{{ $message }}</p>@enderror
                    </div>
                    <button type="submit" class="btn-primary w-full justify-center" wire:loading.attr="disabled">
                        <span wire:loading.remove wire:target="submitKeberatan" class="flex items-center gap-2"><span class="material-symbols-outlined text-base">send</span> Kirim Keberatan</span>
                        <span wire:loading wire:target="submitKeberatan" class="flex items-center gap-2"><span class="material-symbols-outlined text-base animate-spin">progress_activity</span> Mengirim...</span>
                    </button>
                </form>
            </div>
        @endif
    ...
    <div x-show="sub === 'alur_skt'">
        <div class="card p-6 md:p-8">
            ...
            @if ($alurSengketa->content)
                <div class="prose max-w-none text-gray-700 leading-relaxed whitespace-pre-line">{{ $alurSengketa->content }}</div>
            @endif
            @if ($alurSengketa->image)
                <div class="mt-4" x-data="{ showLightbox: false }">
                    <div class="rounded-xl overflow-hidden border border-gray-100 cursor-pointer group relative" @click="showLightbox = true">
                        <img src="{{ Storage::url($alurSengketa->image) }}" alt="Alur Sengketa" class="w-full object-contain max-h-80" loading="lazy">
                        <div class="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors duration-200 flex items-center justify-center">
                            <span class="material-symbols-outlined text-white text-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-200 drop-shadow-lg">zoom_in</span>
                        </div>
                    </div>
                    ...
                    {{-- Lightbox --}}
                    <div x-show="showLightbox" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" @click="showLightbox = false" @keydown.escape.window="showLightbox = false" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 cursor-zoom-out" style="display: none;">
                        <img src="{{ Storage::url($alurSengketa->image) }}" alt="Alur Sengketa" class="max-w-full max-h-[90vh] object-contain rounded-xl shadow-2xl" @click.stop>
                        <button @click="showLightbox = false" class="absolute top-4 right-4 h-10 w-10 rounded-full bg-white/20 hover:bg-white/30 flex items-center justify-center text-white transition-colors">
                            <span class="material-symbols-outlined">close</span>
                        </button>
                    </div>
...
<div x-show="tab === 'maklumat'" x-transition>
    <div class="card p-6 md:p-8">
        ...
        @if ($maklumat->content)
            <div class="prose max-w-none text-gray-700 leading-relaxed whitespace-pre-line">{{ $maklumat->content }}</div>
        @endif
        @if ($maklumat->image)
            <div class="mt-6" x-data="{ showLightbox: false }">
                <div class="rounded-xl overflow-hidden border border-gray-100 cursor-pointer group relative" @click="showLightbox = true">
                    <img src="{{ Storage::url($maklumat->image) }}" alt="Maklumat Pelayanan" class="w-full object-contain max-h-80" loading="lazy">
                    <div class="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors duration-200 flex items-center justify-center">
                        <span class="material-symbols-outlined text-white text-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-200 drop-shadow-lg">zoom_in</span>
                    </div>
                </div>
                {{-- Lightbox --}}
                <div x-show="showLightbox" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" @click="showLightbox = false" @keydown.escape.window="showLightbox = false" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 cursor-zoom-out" style="display: none;">
                    <img src="{{ Storage::url($maklumat->image) }}" alt="Maklumat Pelayanan" class="max-w-full max-h-[90vh] object-contain rounded-xl shadow-2xl" @click.stop>
                    <button @click="showLightbox = false" class="absolute top-4 right-4 h-10 w-10 rounded-full bg-white/20 hover:bg-white/30 flex items-center justify-center text-white transition-colors">
                        <span class="material-symbols-outlined">close</span>
                    </button>
                </div>
    ...
    <div class="space-y-8">
        <div class="card p-6 md:p-8">
            ...
            @if ($jadwalBiaya->content)
                <div class="prose max-w-none text-gray-700 leading-relaxed whitespace-pre-line">{{ $jadwalBiaya->content }}</div>
            @else
        ...
        @if ($jadwalBiaya->attachment)
            <div class="card overflow-hidden">
                <div class="p-5 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 bg-gradient-to-r from-desa-50 to-amber-50">
                    <div class="flex items-center gap-3">
                        <div class="h-10 w-10 rounded-lg bg-gradient-to-br from-red-500 to-red-600 flex items-center justify-center flex-shrink-0">
                            <span class="material-symbols-outlined text-white">picture_as_pdf</span>
                        </div>
                        <div><h3 class="font-bold text-gray-900">Lampiran Jadwal & Biaya</h3></div>
                    </div>
                    <a href="{{ Storage::url($jadwalBiaya->attachment) }}" target="_blank" class="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-desa-500 to-desa-600 hover:from-desa-600 hover:to-desa-700 text-white rounded-xl text-sm font-semibold shadow-md transition-all">
                ...
                <div class="bg-gray-100" style="height: 70vh; min-height: 400px;">
                    <iframe src="{{ Storage::url($jadwalBiaya->attachment) }}#toolbar=1&navpanes=0" class="w-full h-full border-0" loading="lazy"></iframe>
                </div>
            </div>
        @endif
    </div>
...
<div x-show="tab === 'regulasi'" x-transition>
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch">
        {{-- Dasar Hukum --}}
        <div class="card bg-gradient-to-br from-desa-50 to-white border-2 border-desa-100 flex flex-col h-full overflow-hidden">
            <div class="p-6 md:p-8 flex-1">
                ...
                @if ($dasarHukum->content)
                    <div class="prose max-w-none text-gray-700 leading-relaxed whitespace-pre-line">{{ $dasarHukum->content }}</div>
                @else
            ...
            @if ($dasarHukum->attachment)
                <div class="border-t border-gray-100 flex items-center justify-between bg-desa-50/50 px-6 py-4">
                    ...
                    <a href="{{ Storage::url($dasarHukum->attachment) }}" target="_blank" class="text-xs text-desa-600 hover:underline font-medium flex items-center gap-1">
                ...
                <div class="bg-gray-100 border-t border-gray-100" style="height: 50vh; min-height: 300px;">
                    <iframe src="{{ Storage::url($dasarHukum->attachment) }}#toolbar=1&navpanes=0" class="w-full h-full border-0" loading="lazy"></iframe>
                </div>
            @endif
        </div>
        ...
        <div class="card flex flex-col h-full overflow-hidden">
            <div class="p-6 md:p-8 flex-1">
                ...
                @if ($sop->content)
                    <div class="prose max-w-none text-gray-700 leading-relaxed whitespace-pre-line">{{ $sop->content }}</div>
                @else
            ...
            @if ($sop->attachment)
                <div class="border-t border-gray-100 flex items-center justify-between bg-gray-50 px-6 py-4">
                    ...
                    <a href="{{ Storage::url($sop->attachment) }}" target="_blank" class="text-xs text-desa-600 hover:underline font-medium flex items-center gap-1">
                        <span class="material-symbols-outlined text-sm">download</span> Unduh
                    </a>
                </div>
                <div class="bg-gray-100 border-t border-gray-100" style="height: 50vh; min-height: 300px;">
                    <iframe src="{{ Storage::url($sop->attachment) }}#toolbar=1&navpanes=0" class="w-full h-full border-0" loading="lazy"></iframe>
                </div>
            @endif
        </div>
    </div>
