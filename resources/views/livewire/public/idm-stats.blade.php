    ...
    <section class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
        ...
        @if($latest)
            ...
            <div class="card p-8 mb-8 text-center bg-gradient-to-br from-desa-50 to-white border-2 border-desa-100">
                ...
                <span class="inline-block badge text-sm px-5 py-2 {{ $latest->status_color }}">{{ $latest->status_label }}</span>
                <p class="mt-3 text-sm text-gray-500">
                    @if($latest->status === 'mandiri')
                        Selamat! Desa kita telah mencapai status <strong class="text-green-600">Mandiri</strong> — tingkat tertinggi dalam IDM.
                    @elseif($latest->status === 'maju')
                        Desa kita berstatus <strong class="text-blue-600">Maju</strong> dan terus bergerak menuju kemandirian.
                    @elseif($latest->status === 'berkembang')
                        Desa kita berstatus <strong class="text-amber-600">Berkembang</strong> dan sedang dalam proses peningkatan.
                    @else
                ...
                @foreach([
                    ...
                    <div class="card p-5">
                        <div class="flex items-center gap-3 mb-3">
                            <div class="h-10 w-10 rounded-lg bg-{{ $dim['color'] }}-50 flex items-center justify-center">
                                <span class="material-symbols-outlined text-{{ $dim['color'] }}-600">{{ $dim['icon'] }}</span>
                            </div>
                            <div>
                        ...
                        <div class="flex items-end gap-2">
                            <span class="text-3xl font-extrabold text-gray-900">{{ number_format($dim['score'], 3) }}</span>
                            <span class="text-xs text-gray-400 mb-1">/ 1.000</span>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-2 mt-2">
                            <div class="bg-{{ $dim['color'] }}-500 h-2 rounded-full transition-all duration-500" style="width: {{ $dim['score'] * 100 }}%"></div>
                        </div>
                    </div>
                @endforeach
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8" x-data x-init="$nextTick(() => {
                ...
                <div class="card p-6">
                    <h3 class="font-bold text-gray-900 mb-4 flex items-center gap-2">
                        <span class="material-symbols-outlined text-desa-500">radar</span> Dimensi IDM {{ $latest->year }}
                    </h3>
                        ...
                        <tbody>
                            @foreach($allStats as $i => $stat)
                                ...
                                <tr>
                                    ...
                                    <td class="font-mono font-bold text-desa-600">{{ number_format($stat->score, 3) }}</td>
                                    <td><span class="badge {{ $stat->status_color }}">{{ $stat->status_label }}</span></td>
                                    ...
                                    <td>
                                        @if($change !== null)
                                            <span class="inline-flex items-center gap-1 text-sm font-medium {{ $change >= 0 ? 'text-green-600' : 'text-red-600' }}">
                                                <span class="material-symbols-outlined text-sm">{{ $change >= 0 ? 'arrow_upward' : 'arrow_downward' }}</span>
            ...
            <div class="card p-6">
                ...
                <p class="text-sm text-gray-500 mb-4">Status desa ditetapkan berdasarkan rentang skor IDM menurut ketentuan Kementerian Desa, PDT dan Transmigrasi:</p>
                <div class="grid grid-cols-1 sm:grid-cols-5 gap-3">
                    @foreach([
                        ['status' => 'Sangat Tertinggal', 'range' => '< 0,491', 'color' => 'bg-red-100 text-red-800', 'icon' => 'warning'],
                        ['status' => 'Tertinggal', 'range' => '0,491 – 0,599', 'color' => 'bg-orange-100 text-orange-800', 'icon' => 'trending_flat'],
                        ['status' => 'Berkembang', 'range' => '0,600 – 0,707', 'color' => 'bg-amber-100 text-amber-800', 'icon' => 'trending_up'],
                        ['status' => 'Maju', 'range' => '0,708 – 0,815', 'color' => 'bg-blue-100 text-blue-800', 'icon' => 'rocket_launch'],
                        ['status' => 'Mandiri', 'range' => '> 0,815', 'color' => 'bg-green-100 text-green-800', 'icon' => 'stars'],
                    ] as $info)
