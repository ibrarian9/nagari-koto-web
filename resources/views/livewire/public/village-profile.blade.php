<div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-5 gap-10 items-center">
                <div class="lg:col-span-3">
                    ...
                    <p class="mt-3 text-lg text-desa-100 max-w-lg">
                        {{ $village?->tagline ?? 'Membangun nagari maju, sejahtera, dan berbudaya' }}
                    </p>
                    @if ($village?->address)
    ...
    <section class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12" x-data="{ tab: 'sejarah' }">
        <div class="flex flex-wrap justify-center gap-2 mb-8">
            @foreach ([['key' => 'sejarah', 'label' => 'Sejarah', 'icon' => 'history_edu'], ['key' => 'visi', 'label' => 'Visi & Misi', 'icon' => 'flag'], ['key' => 'wilayah', 'label' => 'Wilayah', 'icon' => 'domain'], ['key' => 'peta', 'label' => 'Peta Nagari', 'icon' => 'map']] as $t)
                ...
                <div class="space-y-6">
                    ...
                    <div class="card p-6">
                        ...
                        <div class="space-y-3 text-sm">
                            @foreach ([['label' => 'Provinsi', 'value' => $village?->province ?? '-'], ['label' => 'Kabupaten', 'value' => $village?->regency ?? '-'], ['label' => 'Kecamatan', 'value' => $village?->district ?? '-'], ['label' => 'Kode Nagari', 'value' => $village?->village_code ?? '-']] as $item)
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div class="lg:col-span-2 card p-6 md:p-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        @foreach ([['icon' => 'public', 'label' => 'Provinsi', 'value' => $village?->province ?? '-'], ['icon' => 'apartment', 'label' => 'Kabupaten/Kota', 'value' => $village?->regency ?? '-'], ['icon' => 'domain', 'label' => 'Kecamatan', 'value' => $village?->district ?? '-'], ['icon' => 'tag', 'label' => 'Kode Nagari', 'value' => $village?->village_code ?? '-'], ['icon' => 'landscape', 'label' => 'Luas Wilayah', 'value' => ($village?->area_ha ?? '-') . ' Ha'], ['icon' => 'calendar_month', 'label' => 'Tahun Berdiri', 'value' => $village?->established_year ?? '-']] as $item)
        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col sm:flex-row items-center justify-between gap-6">
                <div>
                    ...
                    <p class="text-sm text-gray-500">Kenali potensi, layanan, dan program nagari kami</p>
                </div>
                <div class="flex flex-wrap gap-3">
                    ...
                    <a href="{{ route('potensi') }}" wire:navigate class="btn-primary btn-sm">
                        <span class="material-symbols-outlined text-base">eco</span> Potensi Nagari
                    </a>
                </div>
            </div>
        </div>
