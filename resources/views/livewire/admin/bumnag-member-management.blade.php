<div>
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div><h2 class="text-xl font-bold text-gray-900">Anggota BUMNag</h2><p class="text-sm text-gray-500 mt-0.5">Kelola data pengurus dan pengawas BUMNag</p></div>
        <button wire:click="create" class="btn-primary btn-sm"><span class="material-symbols-outlined text-base">add</span> Tambah</button>
    </div>

    <x-page-guide title="Panduan Anggota BUMNag" description="Masukkan data pengurus (Direktur, Sekretaris, Bendahara, Kepala Unit) dan pengawas BUMNag. Pilih tipe peran untuk mengelompokkan mereka. Data ini ditampilkan di halaman Struktur Organisasi BUMNag." />

    <x-admin-modal :show="$showForm" :title="($editingId ? 'Edit' : 'Tambah') . ' Anggota BUMNag'" subtitle="Isi data anggota BUMNag" :icon="$editingId ? 'edit' : 'group_add'" iconBg="bg-blue-100" iconColor="text-blue-600">
        <form wire:submit="save" class="space-y-5">
            <x-form-guide>
                <ul class="list-disc list-inside space-y-1">
                    <li><strong>Nama</strong> — Nama lengkap beserta gelar</li>
                    <li><strong>Jabatan</strong> — Posisi di BUMNag (Direktur, Sekretaris, Bendahara, Kepala Unit, Pengawas)</li>
                    <li><strong>Tipe Peran</strong> — Pilih <em>Pengurus</em> untuk pengelola atau <em>Pengawas</em> untuk badan pengawas</li>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div><label class="form-label">Jabatan <span class="text-red-400">*</span></label><input type="text" wire:model="position" class="form-input w-full" placeholder="cth: Direktur">@error('position')<p class="form-error">{{ $message }}</p>@enderror</div>
                <div>
                    <label class="form-label">Tipe Peran <span class="text-red-400">*</span></label>
    ...
    <div class="card overflow-hidden">
        <div class="table-container border-0 shadow-none"><table class="data-table"><thead><tr><th>Urutan</th><th>Foto</th><th>Nama</th><th>Jabatan</th><th>Tipe</th><th>Periode</th><th>Status</th><th class="text-right">Aksi</th></tr></thead><tbody>
            ...
            @empty<tr><td colspan="8" class="text-center py-12"><span class="material-symbols-outlined text-4xl text-gray-200 mb-2">groups</span><p class="text-gray-400 text-sm">Belum ada data.</p></td></tr>@endforelse
        </tbody></table></div>
    </div>
</div>
