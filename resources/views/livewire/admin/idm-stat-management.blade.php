<div>
    ...
    <x-admin-category-modal
        ...
        title="Kelola Kategori Status IDM"
        subtitle="Tambah, ubah, atau hapus kategori status IDM"
        ...
        <form wire:submit="save" class="space-y-5">
            <x-form-guide>
                <ul class="list-disc list-inside space-y-1">
                    ...
                    <li><strong>Skor IDM</strong> — Skor total IDM dari Kemendes (skala 0–635, cth: 550)</li>
                    <li><strong>Status</strong> — Kategori nagari berdasarkan skor</li>
                    <li><strong>Skor Dimensi</strong> — IKS, IKE, IKL, Aksesibilitas, Layanan Dasar, Tata Kelola Pemdes (skala 0–635)</li>
                </ul>
            </x-form-guide>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                ...
                <div><label class="form-label">Skor IDM *</label><input type="number" step="any" min="0" max="635" wire:model="score" class="form-input w-full" placeholder="0 – 635">@error('score')<p class="form-error">{{ $message }}</p>@enderror</div>
            ...
            <div class="border-t border-gray-100 pt-4">
                <p class="text-xs font-bold text-gray-500 uppercase tracking-wider mb-3">Skor Dimensi Utama (0 – 635)</p>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                    <div><label class="form-label">Skor Sosial (IKS)</label><input type="number" step="any" min="0" max="635" wire:model="social_score" class="form-input w-full" placeholder="0 – 635"></div>
                    <div><label class="form-label">Skor Ekonomi (IKE)</label><input type="number" step="any" min="0" max="635" wire:model="economic_score" class="form-input w-full" placeholder="0 – 635"></div>
                    <div><label class="form-label">Skor Lingkungan (IKL)</label><input type="number" step="any" min="0" max="635" wire:model="environment_score" class="form-input w-full" placeholder="0 – 635"></div>
                </div>
            </div>
            <div class="border-t border-gray-100 pt-4">
                <p class="text-xs font-bold text-gray-500 uppercase tracking-wider mb-3">Skor Tambahan (0 – 635)</p>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                    <div><label class="form-label">Skor Aksesibilitas</label><input type="number" step="any" min="0" max="635" wire:model="accessibility_score" class="form-input w-full" placeholder="0 – 635"></div>
                    <div><label class="form-label">Skor Layanan Dasar</label><input type="number" step="any" min="0" max="635" wire:model="basic_service_score" class="form-input w-full" placeholder="0 – 635"></div>
                    <div><label class="form-label">Skor Tata Kelola Pemdes</label><input type="number" step="any" min="0" max="635" wire:model="governance_score" class="form-input w-full" placeholder="0 – 635"></div>
                </div>
            </div>
    ...
    <div class="card overflow-hidden"><div class="table-container border-0 shadow-none"><table class="data-table"><thead><tr><th>Tahun</th><th>Skor</th><th>Status</th><th>IKS</th><th>IKE</th><th>IKL</th><th>Akses.</th><th>Lay. Dasar</th><th>Tata Kelola</th><th class="text-right">Aksi</th></tr></thead><tbody>
        @forelse($stats as $s)<tr><td class="font-medium">{{ $s->year }}</td><td>{{ $s->formatted_score }}</td><td><span class="badge {{ $s->status_color }}">{{ $s->status_label }}</span></td><td>{{ \App\Models\IdmStat::formatIdmScore($s->social_score) }}</td><td>{{ \App\Models\IdmStat::formatIdmScore($s->economic_score) }}</td><td>{{ \App\Models\IdmStat::formatIdmScore($s->environment_score) }}</td><td>{{ \App\Models\IdmStat::formatIdmScore($s->accessibility_score) }}</td><td>{{ \App\Models\IdmStat::formatIdmScore($s->basic_service_score) }}</td><td>{{ \App\Models\IdmStat::formatIdmScore($s->governance_score) }}</td><td><div class="flex justify-end gap-1"><button wire:click="edit({{ $s->id }})" class="h-8 w-8 rounded-lg flex items-center justify-center text-gray-400 hover:bg-desa-50 hover:text-desa-600"><span class="material-symbols-outlined text-lg">edit</span></button><button onclick="confirmAction({{ $s->id }}, 'deleteConfirmed', 'Yakin?')" class="h-8 w-8 rounded-lg flex items-center justify-center text-gray-400 hover:bg-red-50 hover:text-red-600"><span class="material-symbols-outlined text-lg">delete</span></button></div></td></tr>
