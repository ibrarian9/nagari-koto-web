<div>
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div><h2 class="text-xl font-bold text-gray-900">PPID — Informasi Berkala</h2><p class="text-sm text-gray-500 mt-0.5">Kelola dokumen informasi yang dipublikasikan secara berkala</p></div>
    ...
    <x-admin-category-modal
        ...
        title="Kelola Kategori Informasi Berkala"
        subtitle="Tambah, ubah, atau hapus kategori informasi berkala"
    ...
    <div class="card p-6 mb-6">
        ...
        <form wire:submit="save" class="space-y-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    <div class="flex items-center justify-between mb-1"><label class="form-label mb-0">Kategori</label><button type="button" wire:click="openCategoryModal" class="text-xs text-desa-600 hover:underline flex items-center gap-1"><span class="material-symbols-outlined text-xs">add</span> Kelola</button></div>
                ...
                <div><label class="form-label">File Dokumen <span class="text-xs text-gray-400 font-normal">(PDF/DOC/XLS, maks 2MB)</span> {{ $editingId ? '(kosongkan jika tidak ganti)' : '' }}</label><input type="file" wire:model="file" class="form-input w-full text-sm" accept=".pdf,.doc,.docx,.xls,.xlsx">@error('file')<p class="form-error">{{ $message }}</p>@enderror</div>
            </div>
    ...
    <div class="table-container"><table class="data-table"><thead><tr><th>Judul</th><th>Kategori</th><th>Tahun</th><th>File</th><th>Download</th><th>Status</th><th>Aksi</th></tr></thead><tbody>
        ...
        <tr>
            ...
            <td><span class="badge bg-desa-50 text-desa-700 text-xs">{{ $catName }}</span></td>
            ...
            <td class="flex gap-1">
                ...
                <button onclick="confirmAction({{ $item->id }}, 'deleteConfirmed', 'Hapus dokumen ini?')" class="text-red-500 hover:text-red-700"><span class="material-symbols-outlined text-lg">delete</span></button>
            </td>
        </tr>
