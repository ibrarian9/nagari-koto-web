...
class BudgetStatManagement extends Component
{
    ...
    public function edit(int $id): void { $m = BudgetStat::findOrFail($id); $this->editingId = $m->id; $this->fill($m->only(['year','total_income','total_expenditure','realization_pct'])); $this->apbdes_rows = collect($m->apbdes_data ?? [])->map(fn($v,$k)=>['label'=>$k,'value'=>$v])->values()->toArray(); $this->showForm = true; }
    ...
    public function save(): void { $this->validate(); $apbdes = collect($this->apbdes_rows)->pluck('value','label')->filter(fn($v,$k)=>$k!=='')->toArray(); $data = ['year'=>$this->year,'total_income'=>$this->total_income,'total_expenditure'=>$this->total_expenditure,'realization_pct'=>$this->realization_pct,'apbdes_data'=>json_encode($apbdes)]; if ($this->editingId) BudgetStat::findOrFail($this->editingId)->update($data); else BudgetStat::create($data); $this->resetForm(); $this->dispatch('swal', icon: 'success', title: 'Berhasil', text: 'Data anggaran disimpan.'); }
    #[On('deleteConfirmed')]
    ...
    private function resetForm(): void { $this->reset(['showForm','editingId','year','total_income','total_expenditure','realization_pct','apbdes_rows']); $this->year = (int) date('Y'); }
